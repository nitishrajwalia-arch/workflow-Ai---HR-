    ...
    async (req) => {
      ...
      const [
        ...
        canSeeMoney
          ...
          : Promise.resolve([]),
        canSeeMoney ? db.salaryPolicy.findMany() : Promise.resolve([]),
      ...
      const [
        ...
        canSeeMoney
          ? db.vendorInvoice.findMany({ orderBy: { createdAt: 'desc' } })
          : Promise.resolve([]),
        canSeeMoney
          ? db.expense.findMany({ orderBy: { createdAt: 'desc' }, take: 500 })
          : Promise.resolve([]),
        canSeeMoney ? db.sale.findMany({ orderBy: { createdAt: 'desc' } }) : Promise.resolve([]),
        canSeeMoney
          ? db.paymentReminder.findMany({ orderBy: { createdAt: 'desc' }, take: 200 })
          : Promise.resolve([]),
      ...
      for (const h of invHolds)
        heldByItem.set(h.itemName, (heldByItem.get(h.itemName) ?? 0) + h.qty);
      ...
      for (const g of accessGrants)
        ((grants[g.userKey] ??= {})[g.area] ??= {})[g.power] = g.granted;
      ...
      const payload: BootstrapPayload = {
        ...
        // What comes off a payslip, per company. Travels with the salaries: a
        // desk that may not see what somebody earns has no business knowing what
        // is being deducted from them either.
        deductionHeads: deductionHeads.reduce<Record<string, BootstrapDeductionHead[]>>(
          (acc, h) => {
        ...
        salaryPolicies: Object.fromEntries(
          salaryPolicies.map((p) => [
            p.companyId,
            {
              kind: p.kind,
              basicPct: p.basicPct,
              hraPctOfBasic: p.hraPctOfBasic,
              travelPctOfBasic: p.travelPctOfBasic,
              esiEmployeePct: p.esiEmployeePct,
              esiEmployerPct: p.esiEmployerPct,
              esiCeiling: p.esiCeiling,
              pfPct: p.pfPct,
              pfWageCap: p.pfWageCap,
              extraDayDivisor: p.extraDayDivisor,
              setBy: p.setBy,
              setOn: p.setOn,
            },
          ]),
      ...
      return {
        ...
        subs: subs.map((s2) => ({
          id: s2.id,
          title: s2.title,
          fromName: s2.fromName,
          fromDept: s2.fromDept,
          to: s2.to,
          status: s2.status,
        ...
        creditCards: creditCards.map((c) => ({
          ...
          last: c.last,
          holder: c.holder,
          limit: toRupees(c.limit),
          used: toRupees(c.used),
          cycle: c.cycle,
          due: c.due,
          firm: c.firm,
        })),
