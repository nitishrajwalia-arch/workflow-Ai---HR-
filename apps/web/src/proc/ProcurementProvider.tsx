...
export function ProcurementProvider({ children, toast }: Props) {
  ...
  /** Which entry failed, and why — kept so the screen can point at the right one. */
  const [chainFault, setChainFault] = useState<{ index: number; message: string } | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setChainOk(r.ok);
      // Keep the index and the reason. Without them the screen can only say
      // "broken", and it used to say "broken at entry 1" whatever had actually
      // been changed — which points an investigation at an innocent row.
      setChainFault(r.ok ? null : { index: r.index, message: r.message });
      if (!r.ok) toast(r.message, 'red');
    } catch (err) {
  ...
  const value = useMemo<ProcValue | null>(() => {
    ...
    const proc: ProcValue = {
      ...
      devices: w.devices,
      contacts: w.contacts,
      leavePolicy: w.leavePolicy,
      deptRules: w.deptRules,
      ...
      setCoachOn,
      coachRun,
      startCoach: (id: string) => setCoachRun(id),
      ...
      /** `{ index, message }` for the entry that failed, or null when it is sound. */
      chainFault,
      reload: load,
      ...
      addPR: (pr: any) =>
        server(
          () =>
            api.post<any>('/purchase-requests', {
              item: pr.item,
              qty: pr.qty,
              when: pr.when ?? '',
              proj: pr.proj ?? '',
      ...
      fulfillPR: (id: string) =>
        optimistic(
          (x) => ({ ...x, prs: x.prs.filter((p: any) => p.id !== id) }),
      ...
      addVendor: (v: any) =>
        server(
          () => api.post<any>('/vendors', v),
          (r, x) => ({ ...x, vendors: [r, ...x.vendors] }),
        ),
      addVendors: (list: any[]) =>
        server(() => api.post<any>('/vendors/import', { rows: list, commit: true })).then(
          async (r) => {
            if (r) await refetch('vendors', '/vendors');
            return r;
          },
        ),
      verifyVendor: (code: string, patch: any) =>
        server(
          () => api.post<any>(`/vendors/${code}/verify`, patch),
      ...
      addItem: (it: any) =>
        server(
          () => api.post<any>('/inventory', it),
          (r, x) => ({ ...x, inv: [{ ...r, held: 0, available: r.qty }, ...x.inv] }),
        ),
      ...
      adjustItem: (name: string, delta: number, meta: any = {}) =>
        server(
          ...
          (r, x) => ({
            ...
            inv: x.inv.map((i: any) =>
              i.item === name
                ? { ...i, qty: r.newQty, available: Math.max(0, r.newQty - (i.held ?? 0)) }
                : i,
            ),
            moves: [r, ...x.moves],
          }),
            ...
            inv: x.inv.map((i: any) =>
              i.item === r.item
                ? {
                    ...i,
                    held: (i.held ?? 0) + r.qty,
                    available: Math.max(0, i.qty - ((i.held ?? 0) + r.qty)),
                  }
                : i,
            ),
          }),
      ...
      releaseHold: (id: string) =>
        optimistic(
          (x) => {
            ...
            return {
              ...x,
              holds: x.holds.filter((h: any) => h.id !== id),
              inv: gone
                ? x.inv.map((i: any) =>
                    i.item === gone.item
                      ? {
                          ...i,
                          held: Math.max(0, (i.held ?? 0) - gone.qty),
                          available: Math.min(i.qty, (i.available ?? 0) + gone.qty),
                        }
                      : i,
                  )
                : x.inv,
      ...
      setCap: (c: any) =>
        optimistic(
          (x) => ({
            ...x,
            caps: [c, ...x.caps.filter((y: any) => !(y.item === c.item && y.proj === c.proj))],
          }),
          () => api.put('/caps', c),
        ),
      learnItem: (name: string, unit: string, rate: any, vendor: string) => {
        ...
        return optimistic(
          (x) => ({
            ...x,
            catalog: [
              { name: name.trim(), unit, rate: Number(rate) || 0, vendor },
              ...x.catalog.filter((c: any) => c.name.toLowerCase() !== name.trim().toLowerCase()),
            ],
          }),
          () =>
            api.put('/catalog', {
      ...
      markPass: (id: string, patch: any) =>
        optimistic(
          (x) => ({
            ...x,
            gatepasses: x.gatepasses.map((p: any) => (p.id === id ? { ...p, ...patch } : p)),
          }),
      ...
      reviseSub: (id: string, patch: any) =>
        server(
          () =>
            api.post<any>(`/submittals/${id}/revise`, {
      ...
      clearInvoice: (id: string) =>
        optimistic(
          (x) => ({
            ...x,
            invoices: x.invoices.map((v: any) => (v.id === id ? { ...v, state: 'cleared' } : v)),
          }),
      ...
      approveReminder: (idx: number) => {
        ...
        return optimistic(
          (x) => ({
            ...x,
            reminders: x.reminders.map((r: any, i: number) =>
      ...
      registerExport: (e: any) =>
        optimistic(
          (x) => ({ ...x, exports: [e, ...(x.exports ?? []).filter((y: any) => y.key !== e.key)] }),
          () =>
            api.post('/exports', {
              key: e.key,
              title: e.title ?? '',
      ...
      delEvent: (id: string) =>
        optimistic(
          (x) => ({ ...x, events: x.events.filter((e: any) => e.id !== id) }),
          () => api.del(`/events/${id}`),
        ),
      ...
      addPackage: (pk: any) =>
        server(
          () =>
            api.post<any>('/incentives', {
              name: pk.name,
              amount: Number(pk.amount) || 0,
              threshold: Number(pk.threshold) || 0,
              scale: Number(pk.scale) || 10,
              dept: pk.dept ?? '',
              period: pk.period ?? '',
              how: pk.how ?? '',
            }),
      ...
      approvePackage: (id: string) =>
        server(
          () => api.post<any>(`/incentives/${id}/decide`, { action: 'approve' }),
          (r, x) => ({ ...x, packages: x.packages.map((p: any) => (p.id === id ? r : p)) }),
        ),
      declinePackage: (id: string) =>
        server(
          () => api.post<any>(`/incentives/${id}/decide`, { action: 'decline' }),
          (r, x) => ({ ...x, packages: x.packages.map((p: any) => (p.id === id ? r : p)) }),
        ),
      issueBonus: (id: string, who: string) =>
        server(
          () => api.post<any>(`/incentives/${id}/decide`, { action: 'issue', who }),
          (r, x) => ({ ...x, packages: x.packages.map((p: any) => (p.id === id ? r : p)) }),
        ),

      importAtt: (rows: any[], source: string) =>
        server(() => api.post<any>('/attendance/import', { source, rows })).then(async (r) => {
      ...
      addHrTask: (t: any) =>
        server(
          () => api.post<any>('/hr-tasks', { text: t.text, who: t.who ?? '', due: t.due ?? '' }),
      ...
      toggleHrTask: (id: string) =>
        optimistic(
          (x) => ({
            ...x,
            hrTasks: x.hrTasks.map((t: any) => (t.id === id ? { ...t, done: !t.done } : t)),
          }),
      ...
      connect: (k: string) =>
        optimistic(
          (x) => ({ ...x, conns: { ...x.conns, [k]: true } }),
          () => api.post(`/connections/${k}`, { connected: true }),
        ),
      disconnect: (k: string) =>
        optimistic(
          (x) => ({ ...x, conns: { ...x.conns, [k]: false } }),
          () => api.post(`/connections/${k}`, { connected: false }),
        ),
        ...
        optimistic(
          (x) => ({
            ...
            drafts: [
              ...
              ...(x.drafts ?? []).filter((d: any) => d.type !== type),
            ],
          }),
      ...
      dropDraft: (type: string) =>
        optimistic(
          (x) => ({ ...x, drafts: (x.drafts ?? []).filter((d: any) => d.type !== type) }),
          () => api.del(`/drafts/${type}`),
        ),
      ...
      saveGrants: (userKey: string, changes: any[]) =>
        server(
          () => api.post<any>('/access', { userKey, changes }),
      ...
      raiseWithdrawal: (
        ...
        server(() =>
          api.post<any>(`/banks/${accountId}/withdrawal`, { amount, purpose, certifications }),
        ),
      ...
      checkOverride: (code: string, what: string) =>
        api
          ...
          .catch((err) => {
            toast(err instanceof ApiError ? err.message : 'Override refused.', 'red');
            return null;
          }),
      ...
      addDevice: (d: any) =>
        server(
          () => api.post<any>('/devices', d),
          (r, x) => ({ ...x, devices: [r, ...x.devices] }),
        ),
      dropDevice: (id: string) =>
        optimistic(
          (x) => ({ ...x, devices: x.devices.filter((d: any) => d.id !== id) }),
          () => api.del(`/devices/${id}`),
        ),
      setLeave: (dept: string, v: any) =>
        optimistic(
          (x) => ({
            ...x,
            leavePolicy: { ...x.leavePolicy, [dept]: { ...x.leavePolicy[dept], ...v } },
          }),
          () =>
      ...
      setDeptRule: (d: string, r: any) =>
        optimistic(
          (x) => ({ ...x, deptRules: { ...x.deptRules, [d]: { ...x.deptRules[d], ...r } } }),
      ...
      saveCompany: (c: any) =>
        optimistic(
          (x) => ({
            ...x,
            companies: x.companies.some((y: any) => y.id === c.id)
              ? x.companies.map((y: any) => (y.id === c.id ? { ...y, ...c } : y))
              : [...x.companies, c],
          }),
          () => {
            const { id, ...rest } = c;
            return api.put(`/companies/${id}`, rest);
          },
        ),
      saveProject: (p: any) =>
        optimistic(
          (x) => ({
            ...x,
            projects: x.projects.some((y: any) => y.id === p.id)
              ? x.projects.map((y: any) => (y.id === p.id ? { ...y, ...p } : y))
              : [...x.projects, p],
          }),
          () => {
            const { id, ...rest } = p;
            return api.put(`/projects/${id}`, rest);
          },
        ),
      setEmployer: (pid: string, cid: string, reason?: string) =>
        optimistic(
          (x) => ({
            ...x,
            people: x.people.map((p: any) => (p.id === pid ? { ...p, employer: cid } : p)),
          }),
          () =>
      ...
      updatePerson: (id: string, patch: any) =>
        optimistic(
          (x) => ({
            ...x,
            people: x.people.map((p: any) => (p.id === id ? { ...p, ...patch } : p)),
          }),
      ...
      addNote: (id: string, text: string) => {
        ...
        return optimistic(
          (x) => ({ ...x, people: x.people.map((p: any) => (p.id === id ? { ...p, notes } : p)) }),
          () => api.patch(`/people/${id}`, { notes }),
        );
      },
      addPerson: (p: any) =>
        server(
          () => api.post<any>('/people', p),
          (r, x) => ({ ...x, people: [r, ...x.people] }),
        ),
      ...
      bulkAddPeople: async (recs: any[]) => {
        const r = await server(() =>
          api.post<any>('/imports/people', { rows: recs, commit: true }),
        );
        if (!r) return [];
        if (r.rejected?.length)
          toast(
            `${r.accepted.length} added. ${r.rejected.length} held back — see the list.`,
            'amber',
          );
        await load();
        return r.accepted;
      ...
      advanceExit: (id: string, payload: any, summary: string) => {
        ...
        return server(
          () =>
            api.post<any>(`/exits/${id}/advance`, {
              fromStage: cur?.stage ?? 'decision',
              payload: payload ?? {},
              summary,
            }),
        ...
        setWorld((x) =>
          x
            ? {
                ...x,
                hrLog: [
                  { at: new Date().toISOString(), who: w.me?.name ?? '', what: text },
                  ...x.hrLog,
                ],
              }
            : x,
    ...
    return (
      <div className="mb-centre">
        <div className="mb-centre-card">
          ...
          <p>{error}</p>
          <button type="button" className="mb-button" onClick={() => void load()}>
            Try again
          </button>
        </div>
      </div>
